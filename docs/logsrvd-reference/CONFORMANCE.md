<!-- GENERATED by generate-conformance.py from conformance-data.json. Do not edit. -->
...
Nothing is graded `breaking`, `high` or `medium`. Every requirement that named a defect has been closed; what remains is graded `low` or `informational`, and is dominated by `INTENTIONAL` rows — deliberate divergences whose reasons are recorded in the code they describe.
...
| `ARCH-019` | `SIGHUP` reloads configuration and rebuilds listeners witho… | MATCH | none | `internal/server/server.go:406-500 (reload), internal/server…` | — |
...
| `CONF-017` | `-n` suppresses forking, the pid file, and effective stderr… | INTENTIONAL | low | `cmd/sudosrv/main.go:54-73,240-261; packaging/systemd/sudosrv.serv…` | No way to self-background for a sysvinit script; under systemd the behavior equals C's -n mode, which is what C recommends for supervised operation. |
| `CONF-018` | `SIGHUP` performs a full, transactional reload | MATCH | none | `internal/server/server.go` | — |
| `CONF-019` | Listener reconciliation on reload is by literal address str… | INTENTIONAL | informational | `internal/server/listeners.go:96-148` | Reconciliation now matches on the verbatim address string as C does, including the DNS consequence: an unchanged string whose resolution moved does not rebind. Deliberately different in ord… |
...
| `CONF-025` | `[server] timeout` | MATCH | none | `internal/config/config.go, packaging/config/sudosrv.yaml…` | — |
...
| `CONF-033` | `[server] tls_ciphers_v12` and `tls_ciphers_v13` | INTENTIONAL | low | `internal/config/config.go (CipherSuites), internal/server/t…` | tls_ciphers_v12 selects TLS 1.2 suites by IANA name, not as an OpenSSL cipher string -- nothing in Go can parse 'HIGH:!aNULL', so a config written for C fails to load rather than being gues… |
...
| `CONF-055` | `[iolog] iolog_mode` and the derived directory mode | MATCH | none | `internal/config/config.go (DeriveIologModes, EffectiveFileM…` | — |
| `CONF-056` | `[iolog] log_passwords` | MATCH | none | `internal/config/config.go:174; internal/storage/session.go:…` | — |
| `CONF-057` | `[iolog] passprompt_regex` | INTENTIONAL | informational | `internal/config/config.go (ValidatePassPromptRegex), intern…` | Configurable now, with C's semantics: the first configured pattern replaces the default set rather than adding to it, patterns over 1024 bytes are rejected, and any compile failure aborts t… |
| `CONF-058` | `[eventlog] log_type` | MATCH | none | `internal/eventlog/, internal/connection/handler.go (emitEve…` | — |
| `CONF-059` | `[eventlog] log_format` | INTENTIONAL | low | `internal/eventlog/eventlog.go (SudoLine, JSONLine)` | log_format accepts sudo (the default, matching C's field order and octal control-character escaping) and json. Deliberately different for json: this emits one compact object per line, where… |
| `CONF-060` | `[eventlog] log_exit` | MATCH | none | `internal/eventlog/eventlog.go (Logger.Log logExit gate); in…` | — |
| `CONF-061` | `[syslog] maxlen` | MATCH | none | `internal/eventlog/sink.go (splitSyslogLine); internal/confi…` | — |
| `CONF-062` | `[syslog] facility` and `[syslog] server_facility` | INTENTIONAL | informational | `internal/eventlog/sink.go (facilities table); internal/conf…` | facility is implemented with C's table and case-sensitive matching, defaulting to authpriv. Deliberately absent: server_facility, the separate facility for the daemon's OWN warnings. Those… |
| `CONF-063` | `[syslog] accept_priority`, `reject_priority`, `alert_prior… | MATCH | none | `internal/eventlog/sink.go (priorities table, -1 for none);…` | — |
| `CONF-064` | `[logfile] path` | INTENTIONAL | informational | `internal/config/config.go (LogFileConfig); internal/eventlo…` | logfile.path is implemented, opened O_WRONLY\|O_APPEND\|O_CREAT\|O_NOFOLLOW as C does and flock'd per write. Deliberately different for time_format: this takes a Go reference-time layout ra… |
...
| `IOLOG-039` | The password filter applies only to `ttyin`/`ttyout` | MATCH | none | `internal/storage/session.go (writeIoEntry)` | — |
...
| `RELAY-034` | A sub-command Accept during a journalled session reuses the… | MATCH | none | `internal/relay/session.go:591-609; internal/connection/hand…` | — |
...
| `RELAY-042` | A successful replay unlinks the journal and pumps the queue | INTENTIONAL | low | `internal/relay/session.go (flushFile retirement switch, dea…` | The unlink condition now matches: a journal is removed only after the upstream's final commit_point for an I/O session, or after a fully transmitted event-only session, which C treats as co… |
...
| `RELAY-049` | An upstream that permanently rejects a journal causes indef… | INTENTIONAL | informational | `internal/relay/session.go (ErrUpstreamRejected, deadLetter,…` | This requirement documents a C behavior rather than a contract: a journal the upstream answers with an error stays in outgoing/ and is retried on every subsequent start, forever, with no po… |
